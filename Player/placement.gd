...
var RithmaticLines = preload("res://RithmaticLines/rithmatic_lines.gd")
var type : int #RithmaticLines.Type
var p0 : Vector2
var p1: Vector2
var axis_scale: float

func _init(_type: int, _point: Vector2):
	type = _type
	p0 = _point
	p1 = _point
	axis_scale = 1
	
func change_scale(factor: float):
...
func to_polyline() -> Array[Vector2]:
	match type:
		...
		RithmaticLines.Types.WARDING:
			var origin = (p0 + p1) / 2
			var tangent =- (p1 - p0) / 2
			var normal = Vector2(-tangent.y, tangent.x) * axis_scale
			var transform = Transform2D(tangent, normal, origin)
			
			var points : Array[Vector2] = []
			points.resize(RithmaticLines.NUM_WARDING_SEGMENTS + 1)
			
			for i in range(RithmaticLines.NUM_WARDING_SEGMENTS + 1):
				var angle = i * TAU / RithmaticLines.NUM_WARDING_SEGMENTS
		...
		RithmaticLines.Types.VIGOR:
			var origin = p0
			var tangent = p1 - p0
			var normal = RithmaticLines.VIGOR_DEFAULT_BEAM_WIDTH * \
			...
			for i in range(RithmaticLines.NUM_VIGOR_SEGMENTS + 1):
				var x = float(i) / RithmaticLines.NUM_VIGOR_SEGMENTS
